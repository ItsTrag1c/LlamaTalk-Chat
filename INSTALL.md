...
Download `LlamaTalk CLI_X.Y.Z_setup.exe` and run it.
...
Type `/update` inside LlamaTalkCLI. It checks GitHub for a newer version and, if found, downloads and installs it automatically with a live progress bar. No manual file download required.
...
- Cloud API keys are stored in `config.json` in that folder — never shared or exported

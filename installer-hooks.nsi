; Clank Desktop — Custom NSIS installer hooks
...
!macro customInstall
  ...
  Rename "$INSTDIR\uninstall.exe" "$INSTDIR\Clank Desktop_${VERSION}_uninstall.exe"
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCTNAME}" \
    "UninstallString" '"$INSTDIR\Clank Desktop_${VERSION}_uninstall.exe"'
  ...
  FileWrite $R0 "Get-ChildItem -LiteralPath $$d -Filter 'Clank Desktop_*_setup.exe' | Where-Object { $$_.Name -ne '$EXEFILE' } | Remove-Item -Force -ErrorAction SilentlyContinue$\r$\n"
  FileWrite $R0 "Get-ChildItem -LiteralPath $$d -Filter 'Clank Desktop_*_uninstall.exe' | Where-Object { $$_.Name -ne 'Clank Desktop_${VERSION}_uninstall.exe' } | Remove-Item -Force -ErrorAction SilentlyContinue$\r$\n"
  FileClose $R0
...
!macro customUnInstall
  ...
  FileWrite $R0 "$$d = '$INSTDIR'$\r$\n"
  FileWrite $R0 "Get-ChildItem -LiteralPath $$d -Filter 'Clank Desktop_*_setup.exe' | Remove-Item -Force -ErrorAction SilentlyContinue$\r$\n"
  FileWrite $R0 "Get-ChildItem -LiteralPath $$d -Filter 'Clank Desktop_*_uninstall.exe' | Remove-Item -Force -ErrorAction SilentlyContinue$\r$\n"
  FileClose $R0

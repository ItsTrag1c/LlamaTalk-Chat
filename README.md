...
Clank Chat is a Tauri-based desktop app for chatting with [Ollama](https://ollama.com/) models and cloud AI providers — Anthropic Claude, Google Gemini, and OpenAI GPT. Your conversations and settings stay on your machine.
...
| File | Description |
|------|-------------|
| `Clank Chat_x.y.z_aarch64.dmg` | Apple Silicon installer (recommended) |
| `Clank Chat_x.y.z_x64.dmg` | Intel installer |

### Windows

| File | Description |
|------|-------------|
| `Clank Chat_x.y.z_x64-setup.exe` | Windows installer (recommended) |
...
- **Clank Assistant** — floating transparent desktop companion, always on top, draggable (Windows)
...
1. Download the latest DMG from [Releases](https://github.com/ItsTrag1c/Clank-Chat/releases/latest)
2. Open the DMG and drag **Clank Chat** to Applications
...
2. Run the installer — a UAC prompt will appear (installs to `C:\Program Files\Clank Chat\`)
...
Prefer the terminal? **Clank Chat CLI** provides the same chat experience from the command line. Both versions share the same config, profiles, and conversation history.
